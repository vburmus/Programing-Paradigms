...
public class BoundedBuffer implements Produce, Consume {
    ...
    public synchronized int take() {
        ...
        int x = elems[out]; out = (out + 1) % N ;
        n -= 1;
        System.out.println(Thread.currentThread().getName()+" consuming: " + x);
        if (n == N-1) notify();
        return x;
        //W programie sa dwa rodzaje procesów : producent i konsument,
        // obszar krytyczny - bounded buffer.  Problemem jest taka synchronizacja procesów,
        // żeby producent nie dodawał nowych jednostek gdy bufor jest pełny,
        // a konsument nie pobierał gdy bufor jest pusty.
        //
    }
}
